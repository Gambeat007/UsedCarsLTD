...
import pl.gambeat007.ucl.security.role.RoleRepository;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            Role noRestrictions = roleRepository.findByName(UCLRole.PUBLIC).orElseThrow(() ->
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "owner" -> {
                        Role owner = roleRepository.findByName(UCLRole.OWNER).orElseThrow(() ->
                                new RuntimeException("Role not found."));
                        roles.add(owner);
                    }
                    case "customer" -> {
                        Role customer = roleRepository.findByName(UCLRole.CUSTOMER).orElseThrow(() ->
                                new RuntimeException("Role not found."));
                        roles.add(customer);
                    }
                    default -> {
